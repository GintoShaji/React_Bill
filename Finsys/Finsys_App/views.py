...
@api_view(("GET",))
def Fin_get_billVendorData(request):
    try:
        vendorId = request.GET['v_id']
        ven = Fin_Vendor.objects.get(id=vendorId)
        details = {
            'id':ven.id,
            'gstType': ven.GST_Treatment,
            'email': ven.Vendor_email,
            'gstIn': ven.GST_Number if ven.GST_Number else "None",
            'placeOfSupply': ven.Place_of_supply,
            'address': f"{ven.Billing_street},{ven.Billing_city}\n{ven.Billing_state}\n{ven.Billing_country}\n{ven.Billing_pincode}"
        }
        return Response(
...
@api_view(("GET",))
def Fin_fetchPurchaseBill(request, id):
    try:
        data = Fin_Login_Details.objects.get(id=id)
        if data.User_Type == "Company":
            com = Fin_Company_Details.objects.get(Login_Id=id)
        ...
        purchasebill = Fin_Purchase_Bill.objects.filter(Company = com)
        bill = []
        for i in purchasebill:
            ...
            bill.append(obj)
        return Response(
            {"status": True, "purchasebill": bill}, status=status.HTTP_200_OK
        )
    except Exception as e:
        print(e)
        return Response(
